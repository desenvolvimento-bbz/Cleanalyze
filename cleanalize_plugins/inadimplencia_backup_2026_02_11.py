...
def _normalizar_linha_recibo(line: str, debug: bool = False) -> str:
    ...
    """
    m = re.match(
        r'^(\s*\d{5,8})'                    # grupo 1: recibo (5-8 dígitos)
...
def parse_recibo_header(line: str) -> Optional[Dict[str, str]]:
    ...
    # VALIDAÇÃO 2: Emissão deve ter entre 3 e 6 dígitos
    if not (3 <= len(emissao) <= 6):
        _debug_header_stats["headers_rejeitados"] += 1
        return None
...
class Extractor:
    ...
    def extract_records(texto: str, debug: bool = False) -> List[Dict]:
        ...
        for line in linhas:
            ...
            is_continuation_line = not is_main_record_line and bool(re.match(r"^\s*\d+\s+\S", line))

            if is_continuation_line:
                # Extrair conta primeiro para validar conversão OCR
                texto_linha = line.strip()
                m_conta_temp = re.match(r"^(\d+)\s+", texto_linha)
                ...
                if not m_conta:
                    if debug:
                        print(f"[WARN] Não conseguiu extrair conta: {line[:60]}")
                    continue
